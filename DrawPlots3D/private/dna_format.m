function dna_format(tags, trackdata, t0, stage_pos, daqdata, ...
...
if daqdata,
    if size(daq_out, 2) == 2,
        hold on;
        plot(t0, NIDAQ_Out(:, 2)*11.5 + 1.18, 'g'); % Calibration parameters measured on 6/19/08
        hold off;
    end;
end;
axis([0, round(tags{1}(end)), 0, 40]);

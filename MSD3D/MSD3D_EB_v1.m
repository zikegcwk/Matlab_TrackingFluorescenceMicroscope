...
function [dT, DX, DY, DZ,EB,I,Plaser] = MSD3D_EB_v1(varargin);
draw_plot = 0;
...
if nargout>5
        I=atime2bin(tags{1},1e-3,varagin{2},varargin{3});
end
if nargout>6
        Plaser=daq_out;
        n1=1+floor(varaging{2}/dt);
        n2=min(floor(varargin{3}/dt),length(Plaser));
        Plaser=Plaser(n1:n2);
end

function [B,C]=Individual_Molecules_Thermo_V2(kinetics_defmlc_file,fps,minimumlifetime)
...
% Modificaiton were made to plot the fraction of molecules in the molecular
% distribution diatrams.  I Modified the stem plot to indicate the lifetime
% of the molecule not simply its presence
...
tobeimported ='(kinetics_defmlc)cascade11.12.13.14.15.16.17.18.20.21.22.23.24.25.(25 mM)(4).dat';
fps = 10;
minimumlifetime= 60;
...
% single trace summary is a summary carrying 
 tempsummary =[tempfractionhighfret,tracelength,equilibrium,dltg,singlefithigh,stn,singlefitlow,meanfoldingresidual];
...
%axes2 = axes('FontName','Arial','FontSize',10,'Parent',figure1,'Position',[0.15 0.55 0.7 .4],'XTick',[-4 -2 0 2 4],'YAxisLocation','right','box','on','color','none');

ylabel(axes1,'Fraction of Molecules','FontSize',10)
%ylabel(axes2,'Number of Molecule','FontSize',10);

xlabel(axes1,'dlt G Kcal/mol','FontSize',10)
xlim(axes1,[-2.5 2.5]);
ylim(axes1,[0 1]);
...
%bar2=bar((binddata(:,1)+0.25),binddata(:,4),'Parent',axes2,'BarLayout','stacked','BarWidth',1,'EdgeColor',[0 0 0]);
...
%Backup
%stem1=stem(sorted(:,4) ,sorted(:,2),'Parent',axes3,'Marker','none');
...
tracelengthaxess = 300;

axes3 = axes('FontName','Arial','FontSize',10,'Parent',figure1,'Position',[0.15 0.15 0.7 .3],'XTick',[-4 -2 0 2 4],'box','on');
stem1=stem(lowerthantracelength(:,4) ,lowerthantracelength(:,2),'Parent',axes3,'Marker','none','Color','r');
xlim(axes3,[-2.5 2.5]);
ylim(axes3,[0 tracelengthaxess]);
ylabel(axes3,'Trace Length [sec]','FontSize',10);


axes3 = axes('FontName','Arial','FontSize',10,'Parent',figure1,'Position',[0.15 0.15 0.7 .3],'XTick',[-4 -2 0 2 4],'Visible','on','box','on','color','none');
hold(axes3,'all');
stem1=stem(greaterthantracelength(:,4) ,greaterthantracelength(:,2),'Parent',axes3,'Marker','none','Color','k');
xlim(axes3,[-2.5 2.5]);
ylim(axes3,[0 tracelengthaxess]);
ylabel(axes3,'Trace Length [sec]','FontSize',10);

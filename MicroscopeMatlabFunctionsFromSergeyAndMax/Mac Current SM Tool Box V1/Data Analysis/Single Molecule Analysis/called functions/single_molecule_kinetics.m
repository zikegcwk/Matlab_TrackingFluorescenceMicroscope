function data = single_molecule_kinetics(trace_locations,movie_numbers,fps,simulated_traces,...
...
core_file_name = generate_core_file_name(movie_numbers{j},sequential_movies);
% end
...
new_names = track_filename(core_file_name{i},sequential_movies);
...
% tell=size(finaldata) %Sergey debug
% tellmemore=size(finalrawdata)
...
averaged_data = average_and_threshold(finalrawdata,average_method,two_state,threshold_method,threshold_range,x_talk);

finalrawdata = averaged_data{1};
finaldata = averaged_data{2};
end

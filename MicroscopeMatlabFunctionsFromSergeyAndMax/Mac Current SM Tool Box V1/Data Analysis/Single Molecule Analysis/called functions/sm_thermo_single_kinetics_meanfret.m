function [B,C]=sm_thermo_single_kinetics_meanfret(newsorted,qdltg,title,binlim,numberbins,xrange,tracelengthaxess,yrangeunfolding,yrangefolding )
...
axes1 = axes('FontName','Arial','FontSize',10,'Parent',figure2,'Position',[0.1 0.575 0.35 .325],...
...
hold(axes1,'all');
bar1=bar(tempdata(:,1),tempdata(:,3) ,'Parent',axes1,'BarLayout','stacked',...
    'BarWidth',width,'FaceColor',[0 0 0],'EdgeColor',[0 0 0]);

% Plot of molecule that rarely or never fold 
axes0 = axes('FontName','Arial','FontSize',10,'Parent',figure2,'Position',...
    [0.1 0.575 0.35 .325],'box','on','YTick', [0 0.2 0.4 0.6 0.8 1],'XTick',[0],'color','none');
...
hold(axes0,'all');
bar0=bar(extremes(:,1), extremes(:,3),'Parent',axes0,'BarLayout','stacked',...
    'BarWidth',width,'FaceColor','r','EdgeColor','r');
...
axes3 = axes('FontName','Arial','FontSize',10,'Parent',figure2,'Position',[0.1 0.2 0.35 .325],...
    'XTick',dltgtick,'box','on','color','none');
...
hold(axes3,'all');
stem1=stem(dltg ,tracelength,'Parent',axes3,'Color','k','LineStyle','none','MarkerSize',3);
...
annotation1 = annotation(figure2,'textbox','Position',[0.05 .1 0.5 0.03],'FitHeightToText','on',...
    'String',{a,b,c},'LineStyle','none');

axes2 = axes('FontName','Arial','FontSize',10,'Position',[0.6 0.575 0.35 .325],'Parent',figure2,...
    'Visible','on','box','on','Yscale', flodingratescale,'YMinorTick','on','XTick',dltgtick,'color','none');
ylabel(axes2,'Unfolding k [sec^{-1}]');
ylim(axes2,yrangeunfolding);
xlim(axes2,xrange);
hold(axes2,'all');

axes4 = axes('FontName','Arial','FontSize',10,'Position',[0.6 0.2 0.35 .325],'Parent',figure2,...
    'box','on','Yscale', flodingratescale,'YMinorTick','on','XTick',dltgtick,'color','none');
ylabel(axes4,'Folding k [sec^{-1}]');
ylim(axes4,yrangefolding);
xlim(axes4,xrange);
xlabel(axes4,'Mean FRET','FontSize',10)
hold(axes4,'all');

stem2=stem(dltg,unfolding,'Parent',axes2, 'LineStyle','none','MarkerSize',3);
stem4=stem(dltg,folding,'Parent',axes4,'LineStyle','none', 'Marker','square','MarkerEdgeColor',...
    [0 0 1],'MarkerSize',3);
...
annotation(figure2,'textbox','Position',[0.1 .95 0.9 0.03],'FitHeightToText','on','String',...
    strcat(title, ' SM Thermo Mean FRET'),'LineStyle','none');
clipboard('copy',strcat(title, ' SM Thermo Mean FRET'))

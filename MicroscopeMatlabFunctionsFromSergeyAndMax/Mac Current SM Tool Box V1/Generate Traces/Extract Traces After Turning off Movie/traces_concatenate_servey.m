...
% Last Modified by GUIDE v2.5 06-Aug-2008 17:03:05
...
%elseif
    strmatch('tracesp',handles.suffix, 'exact') == 1; %Sergey: to read position-stampted traces
        ...
        handles.newpositions2 = handles.positions2;%Sergey for offsets here
%         size(Data)
%         size(Positions)
...
%Sergey: try it cheap&fast
subplot(handles.axes8); %this is the traces plot
scatter(handles.newpositions2(:,1), handles.newpositions2(:,2),'Marker','*','MarkerEdgeColor','r','SizeData',6);
set(handles.axes8,'xlim', [0 128],'ylim', [0 128],'color','none');
%hold(handles.axes5,'all');
...
%
movie1positions =  [handles.positions1(1:2:end,:) handles.positions1(2:2:end,:)];
movie2positions =  [handles.newpositions2(1:2:end,:) handles.newpositions2(2:2:end,:)];
...
%Sergey introducing offsets

%Sergey done. This will have effect every time "compare" is pressed, so
%watch out.
...
xoffset = str2double(get(handles.edit10,'String'));
yoffset = str2double(get(handles.edit11,'String'));
handles.newpositions2(:,1) = handles.positions2(:,1)+xoffset;
handles.newpositions2(:,2) = handles.positions2(:,2)+yoffset;
guidata(hObject,handles);
Update_Figure(hObject, handles);     
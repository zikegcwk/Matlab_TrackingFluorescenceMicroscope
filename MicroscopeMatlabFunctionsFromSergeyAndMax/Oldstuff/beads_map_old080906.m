...
for k=1:6
 ...
 handles.imagehandle2=imshow(handles.movie_all((max_donx-10):(max_donx+10),(max_dony-10):(max_dony+10)),[50 200]);
 line(don_x(k),don_y(k),'marker','o','color','g','EraseMode','background');

 axes(handles.axes3);
 set(handles.axes3,'CLim',[get(handles.slider1,'min') get(handles.slider1,'value')]);
 handles.imagehandle3=imshow(handles.movie_all((max_accx-10):(max_accx+10),(max_accy-10):(max_accy+10)),[50 200]);
 line(acc_x(k),acc_y(k),'marker','o','color','r','EraseMode','background');
...
low=min(min(K(:,:)));
high=max(max(K(:,:)));
handles.imagehandle=imshow(K(:,:),[low high]);

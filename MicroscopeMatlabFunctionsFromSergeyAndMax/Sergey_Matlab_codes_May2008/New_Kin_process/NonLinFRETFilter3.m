function output = NonLinFRETFilter3(filenumber1,filenumberlast,window,M,p)
%This script will implement G. Harans's (Chem.Phys. 307,137, (2004))
%algorithm for non-liner filtering of FRET data.
%As is, non-linear part works great, but FRET requirement can be stricter.
%Cross-(anti)correlation is not playing enough role. This is because
%weighted by sum of (squared) deviations. When even one deviation is large,
%the sum is large. I can try product, at a risk of dramatically slowing
%down the whole thing. Doesnt slow down, but also not much improvement.
%Clear why: I want to keep things where fluctuations are anti-correlated
%even when they are small, but drop when they are correlated, but large.
%Try adding the deviations before rasing to 2nd power: anticorrelations
%would cancel each other (to some extent, at least) and bring down the
%total deviation. 
%In essense, this makes it a filter for total intensity:)
%Not much better..I see...unless I use very long predictors, I can never
%get rid of blips because they contribute to so many.
...
%          return
    %Data(3,100:120)
    outputfilename = strcat('(NLFF3)(',num2str(window),',',num2str(M),',',num2str(p),')cascade',num2str(ind),'(4).traces');

function varargout = gui_tracecS_Feb08(varargin)
...
handles.crosstalk = 0.08; % Bernie what does this do? It is the crosstalk. SERGEY changes based on bleached state 
handles.first = 1;
handles.last = len;
handles.m = 1;
...
handles.SNR = SNR;
handles.total_I = total_I_av/total_I_middle;
Imax = max(total_I_av);
handles.Imax = Imax;
...
%     tempfilename2=['(FRETDIST' num2str(handles.framerate) ')' handles.filename '.dat'];
...
connectaxes('-x',[handles.axes1 handles.axes2 handles.axes4]);    
% zoom_connect reset;
laseron = find(handles.total_I(40:length(handles.total_I)) >0.65, 1, 'first');
xlimits = get(handles.axes2, 'Xlim');
xlimits(1) = laseron+40;%SERGEYTEMP
...
%     handles.total_I(i)=1;
% end
lightsoff = find(handles.total_I(50:length(handles.total_I)) <0.6, 1, 'first');%SERGEYTEMP
xlimits = get(handles.axes2, 'Xlim');
xlimits(2) = lightsoff+49;%Sergey changed 
set(handles.axes2, 'XLim', xlimits);
% set(handles.axes1, 'XLim', xlimits);
set(handles.axes4, 'XLim', xlimits);%
% zoom_connect reset;
...
fret = get(handles.fret, 'YData');
redoff = find(handles.total_I >1 & fret >0.5, 1, 'last');%SERGEYTEMP
xlimits = get(handles.axes2, 'Xlim');

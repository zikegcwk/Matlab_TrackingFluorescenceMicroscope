function varargout = gui_tracecS_findevents(varargin)
...
handles.crosstalk = 0.15; % Bernie what does this do? It is the crosstalk. SERGEY changed to 0.1 based on bleached state 
handles.first = 1;
handles.last = len;
handles.m = 1;
...
k=left;
p=left;%Sergey to mark the end of the previous dwell
A=zeros(1,5);%Sergey extends array to 6 to write "previous" dwell and next dwell lifetimes
...
for i=left:right-1,
    if tempy(i)~=tempy(i+1)
        ...
        meanfret=mean(fret(k:i)); %Can't this be done easier? (SERGEY)
...
connectaxes('-x',[handles.axes1 handles.axes2 handles.axes4]);    
% zoom_connect reset;
laseron = find(handles.total_I >0.55, 1, 'first');
xlimits = get(handles.axes2, 'Xlim');
xlimits(1) = laseron;%SERGEY deleted -1
...
laseron = find(handles.total_I >0.55, 1, 'first');%Sergey changed here from 0.55 to 0.25 and tries to do better
...
lightsoff = find(handles.total_I <0.25, 1, 'first');
xlimits = get(handles.axes2, 'Xlim');
...
fret = get(handles.fret, 'YData');
redoff = find(handles.total_I >0.3 & fret >0.48, 1, 'last');
xlimits = get(handles.axes2, 'Xlim');

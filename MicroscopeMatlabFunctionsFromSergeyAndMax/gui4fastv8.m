function varargout = gui4fastv8(varargin)


%Sergey June 2008: positions of "good" molecules are niw saved in the same
%traces file (renamed ptraces). The structure of the file now is Nframes,
%Nmols, and then matrix of data(Nmols+1)*(Nframes+2) in which first row is
%frame number (with first and second positions = zero), and for each
%subsequent row position 1 is x coordinate, position 2 is y coordinate, the
%rest are intensities at corresponding frames.

%Sergey Feb 2008
%1)User-defined size of the averaging window for background substraction
%implemented
%2):Background file will be saved with .traces file. The name will be 
%(background'sigma')cascade...(sigma = sd of background fluctuations).traces
%It will contain length in 1st position, Ntraces in 2nd,  
%and average local background substracted from each frame for each
%molecule in "traces" i.e. "background trace". If getting the bg
%flu8ctuations from the name gets old, I'll put it in the body of the file,
%for now I'll keep the structure of .traces files.
...
% GUI4FASTV7 Application M-file for gui4fastv7.fig
%    FIG = GUI4FASTV7 launch gui4fastv7 GUI.
%    GUI4FASTV7('callback_name', ...) invoke the named callback.
...
%SERGEY: here's where it spits out the positions. This is still all of the
%localized molecules, none tossed out.
[x;y];        
...
case 7 % Locate molecules based on acceptor image
    ...
    % out-of-range spots and return the good ones.    
    % By Wei Zhao 03/04/05.
    %SERGEY: this is most likely when out-of-area spots are tossed out... 
    edge_check(handles);
%     debug = handles.i
...
% this will only work when handles.num is defined to be larger than 3
    ...
    filename=strrep(handles.filename,'.pma',['(' num2str(NumPixels) ').tracesp']);%SERGEY_June2008 file name changed here
    filename2 = ['(background' num2str(handles.sigma) ')' filename];
...
% % % %     cd(newdir);
    ...
    % save positions of all molecules to *.mpos file %SERGEY: checked
...
%     tellmemore = size(y)
    ...
    fid2 = fopen(filename2,'w');%This is Sergey's background file
    fwrite(fid2,handles.NumFrames,'int32');
    fwrite(fid2,2*handles.num,'int16');
    
    result=size(handles.image_t);
    temp = repmat(uint16(0), result(1), result(2));
    temp2 = repmat(uint16(0), result(1), result(2));
    traces = zeros(1+2*handles.num,handles.NumFrames+2);
    localbackgrounds = zeros(1+2*handles.num,handles.NumFrames+2);
    %Sergey: fill up first two columns with positions
    for m=1:2*handles.num,
            traces(m+1,1) = x(m);%SERGEY
            traces(m+1,2) = y(m);%SERGEY TOO
            localbackgrounds(m+1,1) = x(m);%SERGEY to keep consistent
            localbackgrounds(m+1,2) = y(m);
    end
    fwrite(fid,traces(1:1+2*handles.num,1),'int16'); 
    fwrite(fid,traces(1:1+2*handles.num,2),'int16');
    fwrite(fid2,localbackgrounds(1:1+2*handles.num,1),'int16');
    fwrite(fid2,localbackgrounds(1:1+2*handles.num,2),'int16');
    
    for k=3:handles.NumFrames+2,
        traces(1,k)=fread(handles.pmafile,1,'uint16');
        localbackgrounds(1,k) = traces(1,k);
...
% has to be indexed in this way: temp(y(m),x(m))
            local_temp=sort(reshape(temp(y(m)-3:y(m)+3,x(m)-3:x(m)+3),49,1));%BACK to 7*7 SERGEYTEMP - down to 5*5
            
% average the middle part as an estimation of the local background.
% 16 = floor(1+7*7); 34 = ceil(1+7*7);
%Sergey_Feb2008: I don't see any reason why middle part should be a good
%estimate of the background. As long as BG is constant, after it was
%substracted in the beginning it remains a gaussian centered at zero.
%Because there is a molecular signal in some of these 49 pixels, the
%average of the middle after BG substraction might end up negative, as
%someone noticied before, and then it is a useless procedure. If the
%background does not stay constant (which is exactly why we try to substract
%it at every frame), the best substraction algorithm depends on the model
%for BG change. If it evenly rises or falls over 7*7 area, taking a mean is
%fine. However, if there are large local fluctuations, they will
%be missed. The question is how to pick up those that affect the signal
%from a molecule, but not those on the edge of 7*7? Think spectrum of
%spatial fluctuations, from 1*1 pixel...2*2...6*6 etc. Only 6*6 and above
%are well caught by current algorithm. Are they really dominant??

            local_background=mean(local_temp(16:34));%SERGEYTEMP
...
%             localbackgrounds(m+1,1) = x(m);%SERGEY to keep consistent
%             localbackgrounds(m+1,2) = y(m);
            localbackgrounds(m+1,k) = NumPixels*local_background;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

        end
        fwrite(fid,traces(1:1+2*handles.num,k),'int16'); 
        fwrite(fid2,localbackgrounds(1:1+2*handles.num,k),'int16');
        set(handles.edit5,'String',int2str(100*k/handles.NumFrames));
        drawnow;
        size(traces);
       
    end
%     tellmeagain = handles.num
    fclose(fid);
    fclose(fid2);
%     cd(current);
...
case 9 % Batch analysis for decimal locations
    ...
    for i=1:number(1),
        ...
        set(handles.popupmenu2,'Value',7); % Locate (SERGEYTEMP) acceptor molecules, decimal precision
        popupmenu2_Callback(handles.popupmenu2, eventdata, guidata(handles.popupmenu2), varargin);
        handles=guidata(handles.popupmenu2);
    ...
    %Sergey - not sure what in this directory change is wrong, but I think this is what causes looping 
    for j=3:number(1),
        if dir_info(j).isdir 
            cd(dir_info(j).name); 
...
z = ones(size(x));
x_range = handles.i - 3;
y_range = handles.j - 3;%SERGEY will fix I for II in conditional statement below
for j=1:2:length(x),
...
delete(findobj(gcf,'type','line'));
% Added Max 021309 to make work well on mac
refresh(gcf)
...
donor_start = str2double(get(handles.edit7,'string'));
acceptor_start = str2double(get(handles.edit8,'string'));

%averagesize=min([30 (NumFrames-max(donor_start,acceptor_start)+1)]);
%%Sergey replaced by user-defined
averagesize = sscanf(get(handles.edit9,'String'),'%d');

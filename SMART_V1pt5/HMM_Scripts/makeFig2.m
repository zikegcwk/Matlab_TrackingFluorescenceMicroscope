...
E = cell([3 1]);
E{1,1} = [mr2; stdr2]; %E{1,2} = [mg1 ; stdg1];
E{2,1} = [mr1; stdr1]; %E{2,2} = [mg2 ; stdg2];
E{3,1} = E{1,1};       %E{3,2} = E{2,2};
...
params.discStates = [1 2];
params.nStates = 3
% params.noHops = [1 3; 3 1];
output3 = TrainPostDec_(params);
output3.BIC = -2*output3.logPx + (6+2*2)*log(length(x));
params.discStates = [1 3];
params.nStates = 4
output4 = TrainPostDec_(params);
output4.BIC = -2*output4.logPx + (12+2*2)*log(length(x));
params.discStates = [1 4];
params.nStates = 5
output5 = TrainPostDec_(params);
output5.BIC = -2*output5.logPx + (20+2*2)*log(length(x));
...
for i = 1:3, 
    plot((pi == i).*((mr1+mr2)/2),'ks','MarkerFaceColor',colors(i,:),'MarkerEdgeColor',colors(i,:),'MarkerSize',3);
end
...
% end
% plot((pi.*0 == 0)-1,'ws','MarkerFaceColor',[1 1 1],'MarkerEdgeColor',[1 1 1]);
plot(output2.postFit(2,:),'r-');
% plot(output2.postFit(2,:),'b-');
axis([0 tMax -0.1 1.1]);
legend('P_1')
ylabel('probability')
...
% end
% plot((pi.*0 == 0)-1,'ws','MarkerFaceColor',[1 1 1],'MarkerEdgeColor',[1 1 1]);
plot(output3.postFit(2,:),'r-');
% plot(output3.postFit(3,:),'g-');
plot(output3.postFit(3,:),'g-');
axis([0 tMax -0.1 1.1]);
legend('P_1','P_2')
ylabel('probability')
...
% end
% plot((pi.*0 == 0)-1,'ws','MarkerFaceColor',[1 1 1],'MarkerEdgeColor',[1 1 1]);
plot(output4.postFit(2,:),'r-');
plot(output4.postFit(3,:),'g-');

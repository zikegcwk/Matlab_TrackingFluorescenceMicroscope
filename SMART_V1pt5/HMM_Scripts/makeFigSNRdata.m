...
SNRmin = 0.1;
SNRmax = 10;
SNRList = logspace(log10(SNRmin),log10(SNRmax),10)';
% SNRList = [1 5 10];
% SNRList = 10;
...
% clear data
% data = cell([length(SNRList) numTrialsPerSNRPoint]);

for snr_idx = 7:length(SNRList),
SNR = SNRList(snr_idx);
N = round(1000/SNR);

clear datatemp
datatemp = cell([numTrialsPerSNRPoint 1]);

for trials_idx = 1:numTrialsPerSNRPoint,
    toc 
    tic
    disp([snr_idx trials_idx length(SNRList) numTrialsPerSNRPoint])
%means and standard deviations of red or green channel in state 1 or 2
muBack = 100; %background count rate

mr1   = muBack;    
mr2   = muBack + fzero(@(d)0.5*(d/sqrt(muBack) + d/sqrt(muBack+d)) - SNR,100);

thresh = fzero(@(x)(x*log(mr1)-mr1-gammaln(x+1) - (x*log(mr2)-mr2-gammaln(x+1)))   ,(mr1+mr2)/2)
...
params.fitChannelType = cell({'poisson','poisson'}); %each channel must be 'exp' or 'gauss'
...
params.SNRwarnthresh = 0.01; %warns if 2 states have SNR less than this
...
params.auto_MeshSpacing = 'auto'; %can be 'square' for equally spaced rectangles  
...
output.piA = GetAfrompi(pi);
%get threshold rate estimate
output.threshA = GetAfrompi((x<thresh).*1 + (x>=thresh).*2);d

output.mr1 = mr1;
output.mr2 = mr2;

% data{snr_idx,trials_idx} = output;
datatemp{trials_idx} = output;

end

for idx2 = 1:numTrialsPerSNRPoint,
    data2{snr_idx,idx2} = datatemp{idx2}; 
end

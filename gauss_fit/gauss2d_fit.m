...
while norm(grad_f) > epsilon_stop & num_iter <= MAX_ITER; 
    ...
    if num_iter > num_gradient_steps,
        ...
        Hess(3,5) = sum(sum(dGdc.*dGde + 16*b*(x-c).*(y-e)/(d^2*f^2).*dGdb .* residual))*dx*dy;
        Hess(3,6) = sum(sum(dGdc.*dGdf + 16*b*(x-c).*(y-e).^2/(d^2*f^3).*dGdb .* residual))*dx*dy;
        ...
        Hess(5,6) = sum(sum(dGde.*dGdf + (16*b*(y-e).^3/f^5 - 8*b*(y-e)/f^3) .* dGdb .* residual))*dx*dy;
        
        Hess(6,6) = sum(sum(dGdf.*dGdf + (16*b*(y-e).^4/f^6 - 12*b*(y-e).^2/f^4).*dGdb .* residual))*dx*dy;
    ...
    tic;
    t = 1;
    while dx*dy*sum(sum((G(x, y, beta + t * step_dir) - z).^2)) > dx*dy*sum(sum(residual.^2)) + line_search_alpha * t * grad_f'*step_dir,
        t = t * line_search_beta;
    end;
    beta = beta + t * step_dir;
    num_iter = num_iter + 1;
    fprintf('after iteration %d, residual: %g\n', num_iter, sum(sum(residual.^2))*dx*dy);
end;

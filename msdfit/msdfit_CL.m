...
function [D_fit, n_fit, gammavec_fit, L] = msdfit_CL(ndim, dt, dx, dy, dz);
if nargin < 3,
...
if num_in == 1,
    semilogx(dt, dx, 'b', dt, g_xy(beta_x, dt), 'k');
        ...
        semilogx(dt, dx, 'b', dt, g_xy(beta_x, dt), 'k', dt, dy, 'g', dt, g_xy(beta_y, dt), 'k', dt, dz, 'r', dt, g_z(beta_z, dt), 'k');
    end;
end;

...
function [D_fit, n_fit, gammavec_fit, L,nd,h] = msdfit_EB(ndim, dt, eb,  dx, dy, dz)


eb=eb/sum(eb);

function y = g_1nw(beta, t);
D = beta(1);
n = beta(2);
gamma_c = beta(3);
...
beta_x = nlinfit(dt, eb.*dx, g_xy, beta_xy);
D_fit(1) = beta_x(1);
n_fit(1) = beta_x(2);
...
if num_in > 1,
    beta_y = nlinfit(dt, eb.*dy, g_xy, beta_xy);
    D_fit(2) = beta_y(1);
    n_fit(2) = beta_y(2);
...
if num_in > 2,
    beta_z = nlinfit(dt, eb.*dz, g_z, beta_z);
    D_fit(3) = beta_z(1);
    n_fit(3) = beta_z(2);
...
if num_in == 1,
    semilogx(dt, dx, 'b', dt, g_xy(beta_x, dt)./eb, 'k');
else if num_in == 2,
        semilogx(dt, dx, 'b', dt, g_xy(beta_x, dt)./eb, 'k', dt, dy, 'g', dt, g_xy(beta_y, dt)./eb, 'k');
    else 
        semilogx(dt, dx, 'b', dt, g_xy(beta_x, dt)./eb, 'k', dt, dy, 'g', dt, g_xy(beta_y, dt)./eb, 'k', dt, dz, 'r', dt, g_z(beta_z, dt)./eb, 'k');
    end;
end;
L = find_L(D_fit, n_fit, gammavec_fit);

% D_noise=n_fit.^2.*gammavec_fit.^2/2
end

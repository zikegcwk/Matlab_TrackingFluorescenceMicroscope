...
function fit = msdfit_zk(dt,dx,dy,dz,beta_i);
...
fit(1).D=beta_estX(1);fit(1).n=beta_estX(2);fit(1).gammaC=beta_estX(3);fit(1).gammaP=beta_estX(4);
fit(1).L = sqrt(fit(1).D * (fit(1).gammaC+fit(1).gammaP) + fit(1).n^2.*fit(1).gammaC/2);

fit(2).D=beta_estY(1);fit(2).n=beta_estY(2);fit(2).gammaC=beta_estY(3);fit(2).gammaP=beta_estY(4);
fit(2).L = sqrt(fit(2).D * (fit(2).gammaC+fit(2).gammaP) + fit(2).n^2.*fit(2).gammaC/2);

fit(3).D=beta_estZ(1);fit(3).n=beta_estZ(2);fit(3).gammaC=beta_estZ(3);fit(3).gammaP=beta_estZ(4);

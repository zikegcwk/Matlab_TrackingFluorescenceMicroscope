...
fit.Lx = sqrt(fit.Dx * (1/fit.gammaCx+1/fit.gammaPx) + fit.nx^2.*fit.gammaCx/2);
fit.Dy=beta_estY(1);fit.ny=beta_estY(2);fit.gammaCy=beta_estY(3);fit.gammaPy=beta_estY(4);
fit.Ly = sqrt(fit.Dy * (1/fit.gammaCy+1/fit.gammaPy) + fit.ny^2.*fit.gammaCy/2);
fit.Dz=beta_estZ(1);fit.nz=beta_estZ(2);fit.gammaCz=beta_estZ(3);fit.gammaPz=beta_estZ(4);
fit.Lz = sqrt(fit.Dz * (1/fit.gammaCz+1/fit.gammaPz) + fit.nz^2.*fit.gammaCz/2);

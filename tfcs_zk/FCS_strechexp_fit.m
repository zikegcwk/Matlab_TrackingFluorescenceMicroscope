function fit=FCS_strechexp_fit(tau,g2,plot_flag)
...
f2fit=@(para,tau) FCS_strechexp(tau,para);
...
if size(g2,1)~=1
    ...
    [tau,ave_g2,std_g2,h]=draw_fcs(tau,g2,1,0);
     ...
     f2fit=@(para,tau) w.^0.5.*FCS_strechexp(tau,para);
     %fitting    
%      figure;plot(log10(tau),wg2);

    [para_fit,r,J,sigma]=nlinfit(tau,wg2,f2fit,para_g);
    %generate c.i. for the fitting parameters
    ci=nlparci(para_fit,r,'jacobian',J);
    %output results
    fit.A=para_fit(1);fit.stdA=ci(1,2)-para_fit(1);
    fit.k=para_fit(2);fit.stdk=ci(2,2)-para_fit(2);
    fit.beta=para_fit(3);fit.stdbeta=ci(3,2)-para_fit(3);
    ...
    plot(log10(tau),FCS_strechexp(tau,para_fit),'r','LineWidth',2);
    legend('data','Streched Exponential fit');Box on;  
...
function g2tau=FCS_strechexp(t,para)
A=para(1);
k=para(2);
beta=para(3);

g2tau=A.*exp(-(k*t).^beta);

...
f2fit=@(para,tau) FCS_threestate(tau,para(1),para(2),para(3),para(4),para(5));
...
if size(g2,1)~=1
    ...
    [tau,ave_g2,std_g2,h]=draw_fcs(tau,g2,1,0);
     ...
     f2fit=@(para,tau) w.^0.5.*FCS_threestate(tau,para(1),para(2),para(3),para(4),para(5));
     %fitting    
%      figure;plot(log10(tau),wg2);

    [para_fit,r,J,sigma]=nlinfit(tau,wg2,f2fit,[para_1_guess,para_2_guess,para_3_guess,para_4_guess,para_5_guess]);
    ...
    fit.tau=tau;
    fit.g2=g2;
    fit.thry=FCS_threestate(tau,para_fit(1),para_fit(2),para_fit(3),para_fit(4),para_fit(5));
    residues=r;
    fit.r=r;
    fit.c=para_fit(5);
    ...
    plot(log10(tau),FCS_threestate(tau,para_fit(1),para_fit(2),para_fit(3),para_fit(4),para_fit(5)),'r','LineWidth',2);
...
function g2tau=FCS_threestate(t,A1,kk1,A2,kk2,c)

g2tau=A1.*exp(-kk1.*t)+A2.*exp(-kk2.*t)+c;

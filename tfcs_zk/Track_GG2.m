%GENERAL INFORAMTION
%%%%%%%%%%%%%%%%%%%%
%function g2tau=Track_GG2(tau,D,bandwidth,noise,laserwaist,model_system,strobing_f,normalize_flag)
%function Tracke_GG2 calculate the autocorrelation functions for a tiny
%object such as a qdot tracked with the 3d tracking system:
%<Track(t)Track(t+tau)>/<Track>^2
...
function g2tau=Track_GG2(tau,fit)

if isfield(fit,'gammaP')
    model_system='second_order';
else
    model_system='first_order';
end
...
laserwaist(1)=0.2;
laserwaist(2)=0.2;
laserwaist(3)=2;%pi*laserwaist(1)^2/0.633;
...
if strobing_f~=0 %if strobing
    ...
    if normalize_flag
       normalize_constant=g2_x(1)*g2_y(1)*g2_z(1)-1;
       %g2_x(1)*g2_y(1)*g2_z(1)-1

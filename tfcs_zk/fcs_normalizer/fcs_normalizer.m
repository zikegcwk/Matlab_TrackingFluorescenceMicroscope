...
function [outx,outy]=fcs_normalizer(f,d,t,output_fig,my_legend)
...
hold all;
for j=length(x):-1:1
    plot((x{j}),y{j}*N(j),'color',c{j},'Marker',mker{j},'MarkerSize',mker_size{j},'MarkerFaceColor',mker_fceC{j},'LineWidth',l_width{j},'LineStyle',style{j});
end
%set(gca,'XScale','log')

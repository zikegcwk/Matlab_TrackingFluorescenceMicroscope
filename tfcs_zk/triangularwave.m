%function triangularwave takes input frequency f and plots a triangular
%wave along time vector tau. 
...
function SB_GG2=triangularwave(tau,f,phase)
...
for j=1:1:(length(tau)-1)
    ...
    for k=1:1:500 %you perhaps will never need to go to 1000!
        ...
        ff(k)=(4/(pi^3*(2*k-1)^3*f*to))*(sin(2*pi*f*(2*k-1)*(tau(j)+to-phase))-sin(2*pi*f*(2*k-1)*(tau(j)-phase)));        
     end
